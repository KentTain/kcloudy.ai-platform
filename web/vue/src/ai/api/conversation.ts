...
export const getConversations = (): Promise<ConversationListResponse> =>
  get<Success<ConversationListResponse>>("/ai/console/v1/conversations").then((res) => res.data);

/**
 * 删除会话
 */
export const deleteConversation = (id: string): Promise<{ success: boolean }> =>
  del<Success<{ success: boolean }>>(`/ai/console/v1/conversations/${id}`).then((res) => res.data);
