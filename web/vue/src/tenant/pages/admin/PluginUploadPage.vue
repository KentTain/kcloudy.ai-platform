...
const handleFileSelect = (file: File) => {
  ...
  if (!isValid) {
    notifyError("请上传 .zip 或 .tar.gz 格式的插件包");
    return;
  }
...
const handleParse = async () => {
  ...
  try {
    ...
    const errorMessage = error?.response?.data?.msg || error?.message || "解析失败";
...
const handleUpload = async () => {
  ...
  try {
    ...
    const errorMessage = error?.response?.data?.msg || error?.message || "上传失败";
    notifyError(errorMessage);
  } finally {
...
<template>
  <div class="flex h-full min-h-0 flex-col gap-4 p-4" data-testid="plugin-upload-page">
    <!-- 页面标题 -->
    <div>
      <h2 class="text-xl font-semibold">上传插件</h2>
    ...
    <Card class="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      >
        <div class="w-full max-w-lg space-y-4">
          ...
          <div class="flex items-center gap-2">
            ...
            />
            <label class="text-sm">覆盖已存在的插件</label>
          </div>
      ...
      <!-- 第二步：预览确认 -->
      <div
        v-if="currentStep === 1 && parsedInfo"
        class="flex min-h-0 flex-1 flex-col overflow-hidden"
      >
        <div class="border-b px-5 py-4">
          <div class="font-medium">插件信息预览</div>
          <div class="text-muted-foreground mt-1 text-xs">
            请确认以下插件信息无误后点击注册
          </div>
        </div>
          ...
          <div class="mx-auto max-w-lg space-y-4">
            ...
            >
              <Checkbox
                ...
                @update:checked="overwrite = $event"
              />
              <label class="text-sm text-amber-800">
                确认覆盖已存在的插件
              </label>
            </div>
          </div>
      ...
      <!-- 第三步：注册结果 -->
      <div
        v-if="currentStep === 2 && uploadResult"
        class="flex min-h-0 flex-1 flex-col overflow-hidden"
      >
        <div class="border-b px-5 py-4">
          <div class="font-medium">注册结果</div>
          <div class="text-muted-foreground mt-1 text-xs">
            插件已成功注册到系统
          </div>
        </div>
