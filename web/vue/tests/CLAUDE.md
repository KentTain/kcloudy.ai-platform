...
详见 [tests/ai/e2e/PLUGIN_TEST_PLAN.md](ai/e2e/PLUGIN_TEST_PLAN.md)。
...
E2E 测试使用两种管理员账号，登录方式和验证流程不同：

| 账号类型 | 账号/密码 | 用途 | Token 存储 |
|---------|----------|------|-----------|
| 租户管理员 | `tenant_admin/admin123` | Tenant 模块管理端测试 | `admin_token` |
| 系统用户 | `admin/admin123` | IAM 模块用户端测试 | `token` |

**重要**：两种账号的登录端点、Token 存储键、权限验证流程均不同，请勿混淆使用。
