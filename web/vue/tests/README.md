...
E2E 测试使用两种管理员账号，分别用于不同的测试场景：

| 账号类型 | 账号/密码 | 用途 | 登录端点 | Token 存储 |
|---------|----------|------|---------|-----------|
| 租户管理员 | `tenant_admin/admin123` | Tenant 模块管理端测试 | `/api/tenant/admin/v1/auth/login` | `admin_token` |
| 系统用户 | `admin/admin123` | IAM 模块用户端测试 | `/api/iam/console/v1/auth/login` | `token` |

**账号区别**：

| 特性 | 租户管理员 | 系统用户 |
|------|-----------|---------|
| 登录端点 | `/api/tenant/admin/v1/auth/login` | `/api/iam/console/v1/auth/login` |
| Token 键名 | `admin_token` | `token` |
| 信息存储 | `admin_info`, `admin_role`, `admin_permissions`, `admin_menus` | `tenant_id`, `refresh_token`, `token_expires_at` |
| 权限范围 | 租户管理、资源配置、模块分配 | 用户管理、角色权限、组织管理 |
| 适用模块 | Tenant 管理端 | IAM 用户端 |

**重要提示**：
- 两种账号的登录验证流程完全不同
- Token 存储键不同，混用会导致认证失败
- 权限范围不同，测试时应使用正确的账号
...
// Tenant 管理端登录（租户管理员）
test('租户管理测试', async ({ page, request }) => {
  await adminLoginViaAPI(page, request, 'tenant_admin', 'admin123');
  // 后续操作...
});

// IAM 用户端登录（系统用户）
test('用户管理测试', async ({ page, request }) => {
...
|------|-----------------|------|
| `ADMIN_TOKEN_KEY` | `admin_token` | 租户管理员认证 Token |
...
| `REFRESH_TOKEN_KEY` | `refresh_token` | 刷新令牌 |
| `TOKEN_EXPIRES_AT_KEY` | `token_expires_at` | Token 过期时间 |
...
```typescript
import { adminLoginViaAPI, createTenantViaAPI, deleteTenantViaAPI, cleanupAllE2EData } from './data-helpers';
