...
- [x] 5.1 修改 `tenant_seed.py` 查询默认资源配置
- [x] 5.2 修改创建默认租户逻辑，关联默认配置 ID
...
- [x] 11.1 编写数据库模型单元测试（`is_default` 字段）
- [x] 11.2 编写种子脚本测试（默认配置创建）
- [x] 11.3 编写服务层测试（`is_default` 唯一性控制）
- [x] 11.4 编写租户创建测试（自动关联默认配置）
- [x] 11.5 编写前端组件测试（配置选择器）
