...
def create_graph(
    ...
    **kwargs,
    ...
    # 此时, _get_node_attributes and _get_edge_attributes have already validated
...
def _clean_value(value: Any) -> str:
    ...
    msg = f"Value 必须是字符串或 None, got {type(value)}"
    raise TypeError(msg)
...
def _get_node_attributes(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if "id" not in mapping and "label" not in mapping and "name" not in mapping:
        msg = "您必须指定 an id, label, or name column in the node attributes"
        raise ValueError(msg)
    return mapping
...
def _get_edge_attributes(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if "source" not in mapping or "target" not in mapping:
        msg = "您必须指定 a source and target column in the edge attributes"
        raise ValueError(msg)
    return mapping
...
def _get_attribute_column_mapping(
    ...
    # 这已经是一个 attribute: column 字典
    """
    获取attribute_column_mapping。

    Args:
        in_attributes (dict[str, Any] | list[str]): in_attributes 参数。
