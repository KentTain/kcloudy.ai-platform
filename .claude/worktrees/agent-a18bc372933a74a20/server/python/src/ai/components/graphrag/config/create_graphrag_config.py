...
from ai.components.graphrag.config.environment_reader import EnvironmentReader
...
def create_graphrag_config(
    ...
    def hydrate_async_type(input: LLMConfigInput, base: AsyncType) -> AsyncType:
        ...
        Args:
            input (LLMConfigInput): input 参数。
            base (AsyncType): base 参数。
    ...
    def hydrate_llm_params(
        ...
        """
        处理hydrate_llm_params。

        Args:
            config (LLMConfigInput): config 参数。
            base (LLMParameters): base 参数。
    ...
    def hydrate_embeddings_params(
        ...
        with reader.use(config.get("llm")):
            ...
            # 在以下几种独特情况下:
            # - LLM和embeddings使用相同的api_base(都是Azure)
            # - LLM和embeddings使用不同的api_base(都是Azure)
            # - LLM使用Azure OpenAI,而embeddings使用基础OpenAI(这一点很重要)
            # - LLM使用Azure OpenAI,而embeddings使用第三方类OpenAI API
            api_base = (
    ...
    def hydrate_parallelization_params(
        ...
        """
        处理hydrate_parallelization_params。

        Args:
            config (LLMConfigInput): config 参数。
            base (ParallelizationParameters): base 参数。
    ...
    with reader.envvar_prefix(Section.graphrag), reader.use(values):
        ...
        with reader.envvar_prefix(Section.input), reader.use(values.get("input")):
            ...
            input_model = InputConfig(
                ...
                secure=reader.bool(Fragment.secure),
                region=reader.str(Fragment.region),
                enable_virtual_style_endpoint=reader.bool(
                    Fragment.enable_virtual_style_endpoint
                ),
            )
        ...
        with reader.use(values.get("reranker")):
            reranker_config = RerankerConfig(
                api_url=reader.str("api_url") or None,
                model=reader.str("model") or defs.RERANKER_MODEL,
                min_score=reader.float("min_score") or defs.RERANKER_MIN_SCORE,
            )
...
def _is_azure(llm_type: LLMType | None) -> bool:
    ...
    Args:
        llm_type (LLMType | None): llm_type 参数。
