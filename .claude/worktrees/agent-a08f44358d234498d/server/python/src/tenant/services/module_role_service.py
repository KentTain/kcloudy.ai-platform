...
class ModuleRoleService:
    ...
    @staticmethod
    async def list_all_by_module(module_id: str) -> list[ModuleRole]:
        """获取模块的所有角色（不分页）"""
        async with async_session() as session:
            stmt = (
                select(ModuleRole)
    ...
    @staticmethod
    async def get_by_id(role_id: str) -> ModuleRole | None:
        """根据 ID 获取角色"""
        async with async_session() as session:
    ...
    @staticmethod
    async def get_by_code(module_id: str, code: str) -> ModuleRole | None:
        """根据模块 ID 和编码获取角色"""
        async with async_session() as session:
    ...
    @staticmethod
    async def get_role_permissions(role_id: str) -> list[ModulePermission]:
        """
        获取角色的权限列表
    ...
    @staticmethod
    async def set_role_all_permissions(role_id: str) -> int:
        """
        为角色设置模块的所有权限
    ...
    @staticmethod
    async def set_role_read_permissions(role_id: str) -> int:
        """
        为角色设置模块的读取权限
    ...
    @staticmethod
    async def get_roles_with_permissions(module_id: str) -> list[dict]:
        """
        获取模块的所有角色及其权限
        ...
        Returns:
            角色列表（包含权限信息）
        """
        roles = await ModuleRoleService.list_all_by_module(module_id)
        result = []
        for role in roles:
