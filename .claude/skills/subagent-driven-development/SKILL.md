...
digraph process {
    ...
    "在 TodoWrite 中标记任务完成" -> "更新计划文档中的 checkbox (- [ ] → - [x])";
    "更新计划文档中的 checkbox (- [ ] → - [x])" -> "还有剩余任务?";
...
**每个任务完成后，必须更新计划文档中的 checkbox 状态。**
...
**如何操作：**
1. 在 TodoWrite 标记任务完成后，立即更新计划文档
2. 使用 Edit 工具将对应任务的 `- [ ]` 替换为 `- [x]`
3. 示例：
   ```markdown
   # 修改前
   - [ ] **步骤 1：创建组件**
   ...
   - [x] **步骤 1：创建组件**
   ```
4. **每完成一个任务就更新一次**，不要等到最后批量更新

**重要：** 更新计划文档的 checkbox 是流程的必需步骤，不是可选的收尾工作。
