...
5. **检查适用的 Superpowers 技能**

   根据任务类型，检查并调用适用的技能：

   | 任务类型 | 应调用的技能 |
   |---------|-------------|
   | 添加新功能/测试 | `test-driven-development` |
   | 修复 Bug | `systematic-debugging` |
   | 创建新组件/模块 | `brainstorming`（如设计不明确） |

   **重要**：在开始实施前，使用 Skill 工具调用适用的技能以获取指导。
...
8. **检查文档更新需求**
   ...
   如果涉及，提示用户是否需要更新文档：
   - 项目根目录 `README.md`
   - `src/demo/CLAUDE.md` 和 `src/demo/README.md`
   - `web/CLAUDE.md` 和 `web/README.md`

9. **验证完成（调用 verification-before-completion）**

   在声称任务完成之前，使用 Skill 工具调用 `verification-before-completion` 技能：
   - 运行验证命令（测试、构建、lint 等）
   - 用证据支撑完成声明
...
- **开始前检查适用的 Superpowers 技能并调用**
- **完成后调用 verification-before-completion 验证**
- 如果任务模糊，在实施前暂停并询问
- 如果实施揭示了问题，暂停并建议更新产物
- 保持代码更改最小并限定在每个任务范围内
- 完成每个任务后立即更新任务复选框
...
- **Superpowers 集成**：在实施前检查适用技能，完成后验证结果
