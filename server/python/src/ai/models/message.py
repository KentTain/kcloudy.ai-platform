...
class Message(BaseModel, ActiveRecordMixin, TenantMixin):
    ...
    conversation_id: Mapped[str] = mapped_column(StringUUID, ForeignKey("ai.conversations.id", ondelete="CASCADE"), nullable=False, index=True, comment="会话id")
    role: Mapped[MessageRole] = mapped_column(EnumType(enum_class=MessageRole), nullable=False, comment="角色")
    content: Mapped[str | None] = mapped_column(Text, nullable=True, comment="消息内容")
    status: Mapped[MessageStatus] = mapped_column(EnumType(enum_class=MessageStatus), nullable=False, default=MessageStatus.PENDING, comment="状态")
    ...
    message_metadata: Mapped[dict | None] = mapped_column(postgresql.JSONB, nullable=True, comment="扩展元数据")
    token_count: Mapped[int | None] = mapped_column(Integer, nullable=True, default=0, comment="token数量")
