...
from ai.components.graphrag.index.graph.extractors.claims import ClaimExtractor
from ai.components.graphrag.index.llm import load_llm
from ai.components.graphrag.index.verbs.covariates.extract_covariates.strategies.graph_intelligence.defaults import (
    MOCK_LLM_RESPONSES,
)
...
async def _execute(
    ...
    Args:
        ...
        texts (Iterable[str]): texts 参数。
        entity_types (list[str]): entity_types 参数。
        resolved_entities_map (dict[str, str]): resolved_entities_map 参数。
        reporter (VerbCallbacks): reporter 参数。
        strategy_config (dict[str, Any]): strategy_config 参数。
