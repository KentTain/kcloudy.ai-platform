"""包含'ClaimExtractorResult'和'ClaimExtractor'模型的模块."""
...
class ClaimExtractor:
    ...
    def _clean_claim(
        ...
        # 清理解析后的声明,删除状态为False的声明
        """
        处理clean_claim。

        Args:
            claim (dict): claim 参数。
            document_id (str): document_id 参数。
            resolved_entities (dict): resolved_entities 参数。
    ...
    async def _process_document(
        ...
        Args:
            prompt_args (dict): prompt_args 参数。
            doc: doc 参数。
            doc_index (int): doc_index 参数。
