...
AI 模块将通过 `document` inner 接口回查源文件权限，在 AI 对话和 RAG 检索中遵守三层权限架构：

| 场景 | 调用方式 | 说明 |
|------|----------|------|
| RAG 检索 | `document/inner/v1/files/check-permission` | 检索前过滤无权限文档 |
| 对话引用 | `document/inner/v1/files/{id}` | 引用来源跳转时校验权限 |

### 依赖 document inner 接口

```python
# AI 模块通过 inner 接口访问 document，不直接依赖 document Schema
from framework.clients.inner_client import get_inner_client

client = get_inner_client()
# 检查用户对文件的访问权限
has_access = await client.post(
    "/document/inner/v1/files/check-permission",
    json={"user_id": user_id, "file_ids": file_ids}
)
```

## 入库审核（Phase 3 预告）

AI 模块将提供文档入库审核能力，与 document 模块协作完成知识库文档的质量审核流程。

## 工具库（Phase 3 预告）

AI 模块将提供工具库能力，支持自定义工具注册、管理和调用。

## 平台设置（Phase 3 预告）

AI 模块将提供平台级设置能力，包括模型配置、安全策略等全局配置管理。
