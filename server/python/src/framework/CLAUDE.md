...
framework ──X──▶ demo / iam / document / ai
```

- 业务模块可以依赖 framework
- framework 禁止依赖业务模块
- 如需业务能力，必须通过 Protocol、注册器或启动期注入实现依赖倒置
- `permission/` 和 `notification/` 通过延迟导入 `iam` 避免循环依赖，禁止在模块加载期直接 import iam
...
| permission/ | 权限判定引擎 Protocol 接口（engine.py）、Policy 求值器（policy_evaluator.py）、审计写入辅助（audit_writer.py） |
| notification/ | 站内信发送辅助（sender.py） |
