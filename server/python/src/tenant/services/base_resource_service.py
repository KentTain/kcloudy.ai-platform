...
class BaseResourceService(ABC):
    ...
    @classmethod
    async def get_by_id(cls, session: AsyncSession, config_id: str) -> Any | None:
        """
        根据 ID 获取配置
    ...
    @classmethod
    async def delete(cls, session: AsyncSession, config_id: str) -> bool:
        """
        删除配置
    ...
    @classmethod
    async def get_default_config(cls, session: AsyncSession) -> Any | None:
        """
        获取当前类型的默认配置
