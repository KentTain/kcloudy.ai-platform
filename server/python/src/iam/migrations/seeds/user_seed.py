...
from iam.models import User, UserStatus, UserRole, UserTenant
...
async def run(*, dry_run: bool = False) -> int:
    ...
    async with get_session() as session:
        ...
        if not tenant_row:
            print("    [WARN] 默认租户不存在，跳过创建系统管理员")
            print("    [HINT] 请先运行 tenant seed 创建默认租户")
            return 0
        ...
        if dry_run:
            print(f"    [DRY-RUN] 将创建用户: {DEFAULT_ADMIN_USERNAME}")
            print(f"    [DRY-RUN] 默认密码: {DEFAULT_ADMIN_PASSWORD}")
            print(f"    [DRY-RUN] 关联租户: {tenant_id}")
            if role_id:
                print(f"    [DRY-RUN] 分配角色: system_admin")
            return 1
        ...
        print(f"    关联租户: {tenant_id}")
        if role_id:
            print(f"    分配角色: system_admin")
