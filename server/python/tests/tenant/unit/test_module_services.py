...
class TestModuleService:
    ...
    @pytest.mark.asyncio
    async def test_list_modules_with_keyword(self, mock_session):
        """带关键词查询模块列表"""
        ...
        items, total = await ModuleService.list_modules(
            mock_session, page=1, page_size=20, keyword="演示"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_by_id_returns_module(self, mock_session):
        """根据 ID 获取模块"""
        mock_module = MagicMock()
        mock_module.id = "module-1"
        mock_module.code = "demo"
        ...
        module = await ModuleService.get_by_id(mock_session, "module-1")

        assert module is mock_module

    @pytest.mark.asyncio
    async def test_get_by_code_returns_module(self, mock_session):
        """根据编码获取模块"""
        mock_module = MagicMock()
        mock_module.id = "module-1"
        mock_module.code = "demo"
        ...
        module = await ModuleService.get_by_code(mock_session, "demo")

        assert module is mock_module

    @pytest.mark.asyncio
    async def test_create_module_with_default_roles(self, mock_session):
        """创建模块时自动创建默认角色"""
    ...
    @pytest.mark.asyncio
    async def test_update_module_success(self, mock_session):
        """更新模块成功"""
        mock_module = MagicMock()
        mock_module.id = "module-1"
        ...
        module = await ModuleService.update(mock_session, "module-1", name="新名称")

        assert module is mock_module
        assert module.name == "新名称"

    @pytest.mark.asyncio
    async def test_update_module_not_found(self, mock_session):
        """更新不存在的模块返回 None"""
        result = MagicMock()
        result.scalar_one_or_none.return_value = None
        mock_session.execute.return_value = result

        module = await ModuleService.update(mock_session, "nonexistent", name="新名称")

        assert module is None

    @pytest.mark.asyncio
    async def test_delete_module_without_references(self, mock_session):
        """无引用时删除模块成功"""
        mock_module = MagicMock()
        mock_module.id = "module-1"
        mock_module.code = "demo"
        ...
        mock_session.execute.side_effect = [module_result, ref_result]
        mock_session.delete = AsyncMock()
        mock_session.commit = AsyncMock()

        success = await ModuleService.delete(mock_session, "module-1")

        assert success is True

    @pytest.mark.asyncio
    async def test_delete_module_with_tenant_module_references_raises_conflict(self, mock_session):
        ...
        with pytest.raises(ValueError) as exc_info:
            await ModuleService.delete(mock_session, "module-1")
...
class TestModuleMenuService:
    ...
    @pytest.mark.asyncio
    async def test_list_menus(self, mock_session):
        """查询模块菜单列表"""
        ...
        menus = await ModuleMenuService.list_menus(mock_session, "module-1")
    ...
    @pytest.mark.asyncio
    async def test_create_menu_success(self, mock_session):
        """创建菜单成功"""
    ...
    @pytest.mark.asyncio
    async def test_update_menu_success(self, mock_session):
        """更新菜单成功"""
        mock_menu = MagicMock()
        mock_menu.id = "menu-1"
        mock_menu.name = "旧名称"
        ...
        with patch("tenant.services.module_menu_service.event_publisher", mock_publisher):
            ...
            menu = await ModuleMenuService.update(mock_session, "menu-1", name="新名称")
    ...
    @pytest.mark.asyncio
    async def test_update_menu_self_parent_raises_error(self, mock_session):
        """菜单不能以自己为父菜单"""
        mock_menu = MagicMock()
        mock_menu.id = "menu-1"
        ...
        with pytest.raises(ValueError) as exc_info:
            await ModuleMenuService.update(mock_session, "menu-1", parent_id="menu-1")

        assert "不能以自己为父菜单" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_delete_menu_with_children_raises_error(self, mock_session):
        """有子菜单时删除抛出错误"""
        # 有 2 个子菜单
        child_result = MagicMock()
        child_result.scalar.return_value = 2
        mock_session.execute.return_value = child_result

        with pytest.raises(ValueError) as exc_info:
            await ModuleMenuService.delete(mock_session, "menu-1")
...
class TestModulePermissionService:
    ...
    @pytest.mark.asyncio
    async def test_list_permissions(self, mock_session):
        """查询模块权限列表"""
        mock_perm = MagicMock()
        mock_perm.id = "perm-1"
        ...
        items, total = await ModulePermissionService.list_permissions(mock_session, "module-1")

        assert len(items) == 1
        assert total == 1

    @pytest.mark.asyncio
    async def test_create_permission_success(self, mock_session):
        """创建权限成功"""
        mock_publisher = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_delete_permission_with_role_reference_raises_conflict(self, mock_session):
        ...
        mock_session.execute.side_effect = [perm_result, ref_result]

        with pytest.raises(ValueError) as exc_info:
            await ModulePermissionService.delete(mock_session, "perm-1")
...
class TestModuleRoleService:
    ...
    @pytest.mark.asyncio
    async def test_list_roles(self, mock_session):
        """查询模块角色列表"""
        mock_role = MagicMock()
        mock_role.id = "role-1"
        ...
        items, total = await ModuleRoleService.list_roles(mock_session, "module-1")
    ...
    @pytest.mark.asyncio
    async def test_update_system_role_raises_error(self, mock_session):
        """更新系统内置角色抛出错误"""
        mock_role = MagicMock()
        mock_role.id = "role-1"
        ...
        with pytest.raises(ValueError) as exc_info:
            await ModuleRoleService.update(mock_session, "role-1", name="新名称")

        assert "系统内置角色" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_delete_system_role_raises_error(self, mock_session):
        """删除系统内置角色抛出错误"""
        mock_role = MagicMock()
        mock_role.id = "role-1"
        mock_role.name = "系统管理员"
        ...
        with pytest.raises(ValueError) as exc_info:
            await ModuleRoleService.delete(mock_session, "role-1")

        assert "系统内置角色" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_delete_non_system_role_success(self, mock_session):
        """删除非系统角色成功"""
        mock_role = MagicMock()
        mock_role.id = "role-1"
    ...
    @pytest.mark.asyncio
    async def test_update_role_permissions(self, mock_session):
        """更新角色权限关联"""
        mock_role = MagicMock()
        mock_role.id = "role-1"
        mock_role.module_id = "module-1"
        ...
        with patch("tenant.services.module_role_service.event_publisher") as mock_publisher:
            ...
            mock_session.execute.side_effect = [role_result, perm_result, existing_result]
            mock_session.commit = AsyncMock()
