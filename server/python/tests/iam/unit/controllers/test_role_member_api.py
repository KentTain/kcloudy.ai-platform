...
class TestGetRoleMembers:
    ...
    @pytest.mark.asyncio
    async def test_get_role_members_success(self):
        """成功获取角色成员列表"""
        mock_members = [
            {
                "user_id": "user-1",
        ...
        with patch(
            ...
            result = await get_role_members("role-123")
    ...
    @pytest.mark.asyncio
    async def test_get_role_members_empty(self):
        """角色无成员时返回空列表"""
        with patch(
            "iam.controllers.admin.role_controller.role_member_service.get_role_members",
            new_callable=AsyncMock,
            ...
            result = await get_role_members("role-empty")
...
class TestAddRoleMembers:
    ...
    @pytest.mark.asyncio
    async def test_add_role_members_success(self):
        """成功添加角色成员"""
        ...
        with patch(
            ...
            result = await add_role_members("role-123", data)
            ...
            assert "成功添加 3 个成员" in response_data["msg"]
            assert response_data["data"]["added"] == 3

    @pytest.mark.asyncio
    async def test_add_role_members_role_not_found(self):
        """角色不存在时抛出 400 错误"""
        from iam.schemas.role import RoleMemberAssignRequest
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc_info:
                await add_role_members("nonexistent-role", data)
...
class TestRemoveRoleMember:
    ...
    @pytest.mark.asyncio
    async def test_remove_role_member_success(self):
        """成功移除角色成员"""
        with patch(
            ...
            result = await remove_role_member("role-123", "user-1")
    ...
    async def test_remove_role_member_not_found(self):
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc_info:
                await remove_role_member("role-123", "nonexistent-user")
...
class TestRoleMenuManagement:
    ...
    @pytest.mark.asyncio
    async def test_get_role_menus_success(self):
        """成功获取角色菜单列表"""
        with patch(
            ...
            result = await get_role_menus("role-123")
    ...
    @pytest.mark.asyncio
    async def test_assign_role_menus_success(self):
        """成功分配角色菜单"""
        ...
        with patch(
            ...
            result = await assign_role_menus("role-123", data)
