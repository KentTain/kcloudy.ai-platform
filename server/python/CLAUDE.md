...
uv run pytest tests/demo/unit/             # 运行单元测试
```
...
配置文件统一放置在 `server/config/` 目录下，实现多技术栈共享配置：
...
1. 安装依赖：`uv sync`
2. 复制配置：`cp server/config/application-local.yml.example server/config/application-local.yml`
