...
- [x] **步骤 3：更新 `server/python/src/framework/CLAUDE.md`**
...
- [x] **步骤 4：更新 `server/python/src/iam/CLAUDE.md`**
...
- [x] **步骤 5：更新 `server/python/src/ai/CLAUDE.md`**

补充预告章节：知识库、入库审核、工具库、平台设置（Phase 3 实现），说明 ai 依赖 document inner 接口回查源文件权限。

- [x] **步骤 6：创建 `server/python/src/document/CLAUDE.md`**
...
- [x] **步骤 5：更新 `__init__.py` 导出 write_audit**
...
- [x] **步骤 3：在 models/__init__.py 导出新模型**
...
- [x] **步骤 2：创建 PermissionRequest 模型**
...
- [x] **步骤 3：在 models/__init__.py 导出**
...
- [x] **步骤 1：在 enums.py 新增 PolicyEffect 枚举**
...
- [x] **步骤 1：阅读现有 organization_service 和 user_service 接口**
...
- [x] **步骤 4：在 user_service.py 增补 search_for_selector**
...
- [x] **步骤 5：在 organization_service.py 增补 get_tree_for_selector**
...
- [x] **步骤 7：在 org_user_controller.py 增补端点**
...
- [x] **步骤 2：创建 permissionRequest 类型**
...
- [x] **步骤 5：创建 permissionRequest API**
...
- [x] **步骤 1：创建 PermissionRequestList 页面**
...
- [x] **步骤 1：编写 NotificationList 组件测试**

测试 store 加载、未读标记、跳转交互（参考现有 iam 组件测试模式）。

- [x] **步骤 2：编写 PolicyList 组件测试**
...
- [x] **步骤 1：在 get_module_definition 增补菜单和权限**
