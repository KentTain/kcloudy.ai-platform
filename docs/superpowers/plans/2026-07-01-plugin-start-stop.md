...
- [ ] **步骤 1：在 `PluginInstallationProvider` 协议中新增 `start_installation` 和 `stop_installation` 方法签名**
...
- [ ] **步骤 1：在 `plugin.py` Schema 文件末尾追加批量启停相关 Schema**
...
- [ ] **步骤 1：在 `PluginInstallationProviderImpl` 中实现 `start_installation` 和 `stop_installation`**
...
- [ ] **步骤 1：创建 `plugin_installation_service.py`**
...
- [ ] **步骤 1：在 controller 顶部添加新的 import**
