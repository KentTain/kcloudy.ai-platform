...
CREATE TABLE tenant.plugin_installations (
    ...
    status          VARCHAR(16) NOT NULL DEFAULT 'PENDING/ACTIVE/INACTIVE/FAILED',
...
| `status` | VARCHAR(16) | **管理** | PENDING/ACTIVE/INACTIVE/FAILED `active`（启用）/ `inactive`（禁用） |
...
class PluginInstallationDTO:
    ...
    plugin_type: str
    status: str                    # PENDING/ACTIVE/INACTIVE/FAILED(管理语义)
    auto_start: bool
...
状态机完整定义
  ...
  │ 当前状态 │   操作   │ 目标状态 │    触发者     │
  ├──────────┼──────────┼──────────┼───────────────┤
  │ -        │ 安装请求 │ PENDING  │ PluginManager │
  ├──────────┼──────────┼──────────┼───────────────┤
  │ PENDING  │ 安装成功 │ ACTIVE   │ PluginManager │
  ├──────────┼──────────┼──────────┼───────────────┤
  │ PENDING  │ 安装失败 │ FAILED   │ PluginManager │
  ├──────────┼──────────┼──────────┼───────────────┤
  │ ACTIVE   │ 禁用     │ INACTIVE │ Web API       │
  ├──────────┼──────────┼──────────┼───────────────┤
  │ INACTIVE │ 启用     │ ACTIVE   │ Web API       │
  └──────────┴──────────┴──────────┴───────────────┘
